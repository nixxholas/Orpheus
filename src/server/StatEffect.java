...
public class StatEffect {
	...
	private List<BuffStatDelta> statups;
	...
	public static StatEffect loadSkillEffectFromData(MapleData source, int skillid, boolean overtime) {
		return loadFromData(source, skillid, true, overtime);
	}

	public static StatEffect loadItemEffectFromData(MapleData source, int itemid) {
		return loadFromData(source, itemid, false, false);
	}

	private static void addBuffStatDeltaToListIfNotZero(List<BuffStatDelta> list, BuffStat buffstat, int val) {
		if (val != 0) {
			list.add(new BuffStatDelta(buffstat, val));
		}
	}

	private static StatEffect loadFromData(MapleData source, int sourceid, boolean skill, boolean overTime) {
		...
		ArrayList<BuffStatDelta> statups = new ArrayList<BuffStatDelta>();
		...
		if (skill) {
			...
			// BEGINNER
				...
				case Legend.RECOVERY:
					statups.add(new BuffStatDelta(BuffStat.RECOVERY, x));
					break;
				case Beginner.ECHO_OF_HERO:
				case Noblesse.ECHO_OF_HERO:
				case Legend.ECHO_OF_HERO:
					statups.add(new BuffStatDelta(BuffStat.ECHO_OF_HERO, ret.x));
					break;
				...
				case Legend.BALROG_MOUNT:
					statups.add(new BuffStatDelta(BuffStat.MONSTER_RIDING, sourceid));
					break;
				case Beginner.BERSERK_FURY:
				case Noblesse.BERSERK_FURY:
					statups.add(new BuffStatDelta(BuffStat.BERSERK_FURY, 1));
					break;
				case Beginner.INVINCIBLE_BARRIER:
				case Noblesse.INVINCIBLE_BARRIER:
				case Legend.INVICIBLE_BARRIER:
					statups.add(new BuffStatDelta(BuffStat.DIVINE_BODY, 1));
					break;
				case Fighter.POWER_GUARD:
				case Page.POWER_GUARD:
					statups.add(new BuffStatDelta(BuffStat.POWERGUARD, x));
					break;
				case Spearman.HYPER_BODY:
				case GM.HYPER_BODY:
				case SuperGM.HYPER_BODY:
					statups.add(new BuffStatDelta(BuffStat.HYPERBODYHP, x));
					statups.add(new BuffStatDelta(BuffStat.HYPERBODYMP, ret.y));
					break;
				case Crusader.COMBO:
				case DawnWarrior.COMBO:
					statups.add(new BuffStatDelta(BuffStat.COMBO, 1));
					break;
				...
				case ThunderBreaker.LIGHTNING_CHARGE:
					statups.add(new BuffStatDelta(BuffStat.WK_CHARGE, x));
					break;
				case DragonKnight.DRAGON_BLOOD:
					statups.add(new BuffStatDelta(BuffStat.DRAGONBLOOD, ret.x));
					break;
				...
				case Aran.FREEZE_STANDING:
					statups.add(new BuffStatDelta(BuffStat.STANCE, iprop));
					break;
				case DawnWarrior.FINAL_ATTACK:
				case WindArcher.FINAL_ATTACK:
					statups.add(new BuffStatDelta(BuffStat.FINALATTACK, x));
					break;
				// MAGICIAN
				case Magician.MAGIC_GUARD:
				case BlazeWizard.MAGIC_GUARD:
					statups.add(new BuffStatDelta(BuffStat.MAGIC_GUARD, x));
					break;
				case Cleric.INVINCIBLE:
					statups.add(new BuffStatDelta(BuffStat.INVINCIBLE, x));
					break;
				case Priest.HOLY_SYMBOL:
				case SuperGM.HOLY_SYMBOL:
					statups.add(new BuffStatDelta(BuffStat.HOLY_SYMBOL, x));
					break;
				case FPArchMage.INFINITY:
				case ILArchMage.INFINITY:
				case Bishop.INFINITY:
					statups.add(new BuffStatDelta(BuffStat.INFINITY, x));
					break;
				case FPArchMage.MANA_REFLECTION:
				case ILArchMage.MANA_REFLECTION:
				case Bishop.MANA_REFLECTION:
					statups.add(new BuffStatDelta(BuffStat.MANA_REFLECTION, 1));
					break;
				case Bishop.HOLY_SHIELD:
					statups.add(new BuffStatDelta(BuffStat.HOLY_SHIELD, x));
					break;
				// BOWMAN
				...
				case WindArcher.SOUL_ARROW:
					statups.add(new BuffStatDelta(BuffStat.SOULARROW, x));
					break;
				case Ranger.PUPPET:
				case Sniper.PUPPET:
				...
				case Corsair.WRATH_OF_THE_OCTOPI:
					statups.add(new BuffStatDelta(BuffStat.PUPPET, 1));
					break;
				case Bowmaster.CONCENTRATE:
					statups.add(new BuffStatDelta(BuffStat.CONCENTRATE, x));
					break;
				case Bowmaster.HAMSTRING:
					statups.add(new BuffStatDelta(BuffStat.HAMSTRING, x));
					monsterStatus.put(MonsterStatus.SPEED, Integer.valueOf(x));
					break;
				case Marksman.BLIND:
					statups.add(new BuffStatDelta(BuffStat.BLIND, x));
					monsterStatus.put(MonsterStatus.ACC, Integer.valueOf(x));
					break;
				case Bowmaster.SHARP_EYES:
				case Marksman.SHARP_EYES:
					final int delta = ret.x << 8 | ret.y;
					statups.add(new BuffStatDelta(BuffStat.SHARP_EYES, delta));
					break;
				// THIEF
				case Rogue.DARK_SIGHT:
				case WindArcher.WIND_WALK:
				case NightWalker.DARK_SIGHT:
					statups.add(new BuffStatDelta(BuffStat.DARKSIGHT, x));
					break;
				case Hermit.MESO_UP:
					statups.add(new BuffStatDelta(BuffStat.MESOUP, x));
					break;
				case Hermit.SHADOW_PARTNER:
				case NightWalker.SHADOW_PARTNER:
					statups.add(new BuffStatDelta(BuffStat.SHADOWPARTNER, x));
					break;
				case ChiefBandit.MESO_GUARD:
					statups.add(new BuffStatDelta(BuffStat.MESOGUARD, x));
					break;
				case ChiefBandit.PICKPOCKET:
					statups.add(new BuffStatDelta(BuffStat.PICKPOCKET, x));
					break;
				case NightLord.SHADOW_STARS:
					statups.add(new BuffStatDelta(BuffStat.SHADOW_CLAW, 0));
					break;
				// PIRATE
				case Pirate.DASH:
				...
				case Noblesse.SPACE_DASH:
					statups.add(new BuffStatDelta(BuffStat.DASH2, ret.x));
					statups.add(new BuffStatDelta(BuffStat.DASH, ret.y));
					break;
				case Corsair.SPEED_INFUSION:
				case Buccaneer.SPEED_INFUSION:
				case ThunderBreaker.SPEED_INFUSION:
					statups.add(new BuffStatDelta(BuffStat.SPEED_INFUSION, x));
					break;
				case Outlaw.HOMING_BEACON:
				case Corsair.BULLSEYE:
					statups.add(new BuffStatDelta(BuffStat.HOMING_BEACON, x));
					break;
				case ThunderBreaker.SPARK:
					statups.add(new BuffStatDelta(BuffStat.SPARK, x));
					break;
				// MULTIPLE
				...
				case ThunderBreaker.KNUCKLER_BOOSTER:
					statups.add(new BuffStatDelta(BuffStat.BOOSTER, x));
					break;
				...
				case Aran.MAPLE_WARRIOR:
					statups.add(new BuffStatDelta(BuffStat.MAPLE_WARRIOR, ret.x));
					break;
				// SUMMON
				case Ranger.SILVER_HAWK:
				case Sniper.GOLDEN_EAGLE:
					statups.add(new BuffStatDelta(BuffStat.SUMMON, 1));
				...
				case Aran.COMBO_ABILITY:
					statups.add(new BuffStatDelta(BuffStat.ARAN_COMBO, 100));
					break;
				case Aran.COMBO_BARRIER:
					statups.add(new BuffStatDelta(BuffStat.COMBO_BARRIER, ret.x));
					break;
				case Aran.COMBO_DRAIN:
					statups.add(new BuffStatDelta(BuffStat.COMBO_DRAIN, ret.x));
					break;
				case Aran.SMART_KNOCKBACK:
					statups.add(new BuffStatDelta(BuffStat.SMART_KNOCKBACK, ret.x));
					break;
				case Aran.BODY_PRESSURE:
					statups.add(new BuffStatDelta(BuffStat.BODY_PRESSURE, ret.x));
					break;
				case Aran.SNOW_CHARGE:
					monsterStatus.put(MonsterStatus.SPEED, Integer.valueOf(ret.x));
					statups.add(new BuffStatDelta(BuffStat.WK_CHARGE, ret.y));
					break;
				default:
					break;
			}
		...
		if (ret.isMorph()) {
			statups.add(new BuffStatDelta(BuffStat.MORPH, ret.getMorph()));
		}
		if (ret.ghost > 0 && !skill) {
			statups.add(new BuffStatDelta(BuffStat.GHOST_MORPH, ret.ghost));
		}
		...
		if (moveTo != -1) {
			if (applyto.getMap().getReturnMapId() != applyto.getMapId()) {
				...
				if (moveTo == 999999999) {
					...
					target = applyto.getClient().getWorldServer().getChannel(applyto.getClient().getChannelId()).getMapFactory().getMap(moveTo);
	...
	public final void applyComboBuff(final GameCharacter applyto, int combo) {
		final List<BuffStatDelta> stat = Collections.singletonList(new BuffStatDelta(BuffStat.ARAN_COMBO, combo));
	...
	private void applyBuffEffect(GameCharacter applyfrom, GameCharacter applyto, boolean primary) {
		...
		List<BuffStatDelta> localstatups = statups;
		int localDuration = duration;
		...
		if (isMonsterRiding()) {
			...
			localstatups = Collections.singletonList(new BuffStatDelta(BuffStat.MONSTER_RIDING, 0));
		} else if (isSkillMorph()) {
			localstatups = Collections.singletonList(new BuffStatDelta(BuffStat.MORPH, getMorph(applyto)));
		}
		if (primary) {
		...
		if (localstatups.size() > 0) {
			...
			if (isDash()) {
				...
				List<BuffStatDelta> dsstat = Collections.singletonList(new BuffStatDelta(BuffStat.DARKSIGHT, 0));
				...
				List<BuffStatDelta> stat = Collections.singletonList(new BuffStatDelta(BuffStat.SHADOWPARTNER, 0));
				mbuff = PacketCreator.giveForeignBuff(applyto.getId(), stat);
			} else if (isSoulArrow()) {
				List<BuffStatDelta> stat = Collections.singletonList(new BuffStatDelta(BuffStat.SOULARROW, 0));
				mbuff = PacketCreator.giveForeignBuff(applyto.getId(), stat);
			} else if (isEnrage()) {
	...
	private StatEffect getAlchemistEffect(GameCharacter chr) {
	...
	private static class CancelEffectAction implements Runnable {
		...
		public CancelEffectAction(GameCharacter target, StatEffect effect, long startTime) {
			this.effect = effect;
	...
	public List<BuffStatDelta> getStatups() {
		return statups;
	}
